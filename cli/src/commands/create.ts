...
export const createCommand = new Command('create')
  ...
  .action(async (options) => {
    try {
      ...
      // Scaffold project
      let projectPath = path.resolve(options.output, options.name || projectData.name);
      const expo = new ReactNativeExpoScaffold();
      const result = await expo.run({ projectName: options.name || projectData.name, destination: options.output });

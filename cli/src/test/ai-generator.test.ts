...
describe('AI generator', () => {
	it('replaces HTML DYNAMIC comments with content (local provider)', async () => {
		...
		const out = await generateWithAI(input, baseContext, 'docflow/test.md', { aiProvider: 'local', research: false, dryRun: false, output: '/tmp' });
	...
	it('falls back to local when API keys missing', async () => {
		...
		const out = await generateWithAI(input, baseContext, 'docflow/test.md', { aiProvider: 'openai', research: false, dryRun: false, output: '/tmp' });
		expect(out).not.toContain('<!-- DYNAMIC:');
		expect(out).toContain('Generated locally');
	});
});
